(function (angular, undefined) {
	...
	module.value('customSelectDefaults', {
		...
		emptySearchResultText: 'Not found '+'"$0"',
		addText: 'Add',
		searchDelay: 300
	});
				...
				var itemTemplate = elem.html().trim() || '{{' + (match[2] || match[1]) + '}}',
					...
					'<div class="dropdown-menu">' +
						'<div stop-propagation="click" class="custom-select-search">' +
							'<input class="' + attrs.selectClass + '" type="text" autocomplete="off" ng-model="searchTerm" tabindex="-1" />' +
						'</div>' +
						'<ul role="menu">' +
							'<li role="presentation" ng-repeat="' + valueName + ' in matches' + trackByExpr + '">' +
								'<a role="menuitem" class="select-item" tabindex="-1" href ng-click="select(' + valueName + ')">' +
									itemTemplate +
								'</a>' +
							'</li>' +
