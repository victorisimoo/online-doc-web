...
<head>
    ...
    <title>Dr Online - Chat privado de paciente</title>
    ...
    <script src="libs/quickblox.min.js?random=<?php echo uniqid(); ?>" defer></script>
    <!--El atributo defer mejora el tiempo de renderizado, descargando en paralelo y ejecutando al final-->
</head>
<body>
    <button id="closeViewer" class="btn btn-default" style="position: fixed; bottom: 10px; right: 10px; z-index: 1001; display: none">Cerrar</button>
    <div id="page"></div>
    ...
    <script type="text/template" id="tpl_dashboardContainer">
        <div class="dashboard"> <!--Contiene todo lo que genera QB, LA RAIZ-->
            <div class="sidebar j-sidebar"> <!--SIDEBAR que contiene a los participantes-->
            ...
            <div class="content j-content active"><!--CONTIENE EL HISTORIAL DE CHATS, ACA LO EMBEBE-->
            </div>
        </div>
    </script>
    ...
    <script type="text/template" id="tpl_conversationContainer">
        ...
        <div class="content__title j-content__title j-dialog">
            <% if (doctorName) { %>
                <h1 class="dialog__title j-dialog__title"><%- doctorName %></h1>
            <% } else { %>
                <h1 class="dialog__title j-dialog__title"><%- title %></h1>
            <% } %>
            <!--IMPRIME EL NOMBRE DEL OPONENTE-->
            <div class="action_links">
                <a href="#!/dialog/<%- _id %>/edit" class="add_to_dialog j-add_to_dialog <% type !== 2 ? print('hidden') : ''%>">
                    <i class="material-icons">person_add</i>
                </a>
                <a href="#" class="quit_fom_dialog_link j-quit_fom_dialog_link hidden" data-dialog="<%- _id %>">
        ...
        <div class="content__inner j-content__inner">
            ...
            <form name="send_message" class="send_message" autocomplete="off"> 
                    <textarea name="message_feald" class="message_feald" id="message_feald" autocomplete="off"
                              autocorrect="off" autocapitalize="off" placeholder="<%- parent.translationPrefered.CHAT_SEND_MESSAGE %>" autofocus></textarea>
                <div class="message__actions">
                    ...
                    <button class="send_btn"><i class="material-icons">send</i></button>
                </div>
            </form>
        </div>
    </script>
            ...
            <div class="message__content"> <!--TEXTO DEL MENSAJE-->
                <div class="message__text_and_date">
                    <div class="message__text_wrap" style='padding: 6px; background-color: #eee; border-radius: 6px;'>
                        ...
                        <% } %>
                        <div class="message__timestamp"> <!--TIEMPO EN EL QUE FUE RECIBIDO -->
                            <%= message.date_sent %>
                        </div>
                    </div>
                </div>
            ...
            <div class="message__content" style='margin-right: 8px; text-align: right;'> <!--TEXTO DEL MENSAJE-->
                <div style="display: block; width: auto;">
                    <div class="message__text_wrap">
                        ...
                        <% } %>
                        <div class="message__timestamp_me" style="float: right; display: inline-flex;"> <!--TIEMPO EN EL QUE FUE RECIBIDO -->
                            <p style='opacity: 0.6;'><%= message.date_sent %></p><p><i class="material-icons message__status j-message__status <% message.status.seen ? print('seen') : ''%>" style="font-size: 20px; color: white; padding-left: 3px; opacity: 0.6;"><%= message.status.status %></i></p>
                        </div>
                    </div>
                </div>
                <div>
    ...
    <script type="text/template" id="tpl_loading">
        <div class="loading__wrapper">
            <div class="loading_inner">
                <p id='load' class="loading__description"><%- parent.translationPrefered.LOADING_MSG%></p>
            </div>
        </div>
    </script>
    ...
    <script src="js-patient/QBconfig-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/user-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/dialog-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/message-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/listeners-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/helpers-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/app-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/login-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <script src="js-patient/route-n.js?random=<?php echo uniqid(); ?>" defer></script>
    <?php ob_end_flush(); ?>
</body>
</html>
