...
function App(config) {
    this._config = config; //Variable que guarda valores de configuracion del archivo QBConfig.js
    this.user = null;
    this.token = null;
    this.isDashboardLoaded = false; // Variable para ver si se ha rendezidado el dashboard
    this.room = null; //Variable que guarda la room
    // Elements
    this.page = document.querySelector('#page');//Toma el primer elemento con ese ID
    this.sidebar = null;
    this.content = null;
...
App.prototype.init = function (config) {
    ...
    //Inicializa QB con las credenciales.
...
App.prototype.renderDashboard = function (activeTabName) {
    ...
    var params = {
        type: 3,
        occupants_ids: resQB.patient.QBId,
        medicalId: resQB.medicalAppointmentId,
        patientName: resQB.patient.name
    };
...
bindEvent(window, 'message', function (e) {
    ...
    if(e.data.reason == 1){
        var params = {
            type: 3,
            occupants_ids: e.data.patient,
            medicalId: e.data.medicalId,
            patientName: e.data.patientName
        };
        logInfo(e.data.medicalId, "DRONLINE DOCTOR: Se recibió petición de creación de chat en iframe.");
        dialogModule.createDialog(params);
    }
    else if(e.data.reason == 2){
        //Remove dialog from screen and select another
        var params = {
            type: 3,
            occupants_ids: e.data.patient,
            medicalId: e.data.medicalId,
        ...
        logInfo(e.data.medicalId, "DRONLINE DOCTOR: Removiendo el chat de la estructura de Quickblox.");
        dialogModule.removeDialog(params);
    }
    else if(e.data.reason == 3){
        doctorPhoto = e.data.photo;
    }
});
