var userQBId = [];
var dialogsList = [];
var patientsListNames = [];
var medicalIdList = [];
var actualMedicalId = null;
var doctorPhoto = "img/profile-pics/4.jpg";
'use strict';
...
Dialog.prototype.renderDialog = function (dialog, setAsFirst) {
    ...
    dialog.last_message = (dialog.last_message == '[attachment]') ? '[imagen]' : dialog.last_message;
    dialog.name = patientsListNames[dialog._id];
...
Dialog.prototype.joinToDialog = function (id) {
    infoLog("Dialog ID: "+id);
    var self = this,
        jidOrUserId = self._cache[id].jidOrUserId;

        infoLog("jidOrUserId: "+jidOrUserId);
    return new Promise(function (resolve, reject){
        QB.chat.muc.join(id, function (resultStanza) {
...
Dialog.prototype.renderMessages = function (dialogId) {
    ...
    if (!document.forms.send_message) {
        ...
        self.content.innerHTML = helpers.fillTemplate('tpl_conversationContainer', {title: patientsListNames[dialog._id], _id: dialog._id, type: dialog.type});
...
Dialog.prototype.changeLastMessagePreview = function (dialogId, msg) {
    ...
    if (dialog) {
        ...
        if (msg.message) {
            ...
            messagePreview.innerText = parent.translationPrefered.ATTACHMENT_MESSAGE;
        }
...
Dialog.prototype.createDialog = function (params) {
    ...
    QB.chat.dialog.create(params, function (err, createdDialog) {
        if (err) {
            logSevere(params.medicalId, "DRONLINE DOCTOR: Error creando chat: "+JSON.stringify(err));
            console.error(err);
        } else {
            logInfo(params.medicalId, "DRONLINE DOCTOR: Chat creado exitosamente.");
            logInfo(params.medicalId, "DRONLINE DOCTOR: DialogId: <b>"+createdDialog._id+"</b> - Patient QBId: <b>"+params.occupants_ids+"</b>");
            var occupants_names = [],
            ...
            _.each(occupants, function (occupantId) {
                var occupant_name = userModule.getUserById(params.occupants_ids).name;

                occupants_names.push(occupant_name);
            });
            userQBId[id] = params.occupants_ids;
            dialogsList[params.medicalId] = createdDialog;
            medicalIdList[id] = params.medicalId;
            patientsListNames[id] = params.patientName;
            createdDialog.name = params.patientName;
            infoLog(dialogsList);
            ...
            for (var i = 0; i < newOccupantsIds.length; i++) {
                logInfo(params.medicalId,"DRONLINE DOCTOR: Enviando notificación de la creación del chat al paciente.");
                QB.chat.sendSystemMessage(newOccupantsIds[i], systemMessage);
            }

            self.updateMedicalAppointmentRoomId(params.medicalId, createdDialog._id);
...
Dialog.prototype.removeDialog = function(params){
    var self = this;
    infoLog(params);
    var dialogId = dialogsList[params.medicalId];
    delete self._cache[dialogId._id];
    var dialogElem = document.getElementById(dialogId._id);
    dialogElem.parentNode.removeChild(dialogElem);
    delete dialogsList[params.medicalId];
    var msg = {
        body: 'disconnect', 
        type: 'chat',
        extension: {
            notification_type: CONSTANTS.NOTIFICATION_TYPES.DISCONNECT,
            dialog_id: dialogId._id
        }
    };        
    QB.chat.sendSystemMessage(params.occupants_ids, msg);    
    var newDialog = dialogsList[Object.keys(dialogsList)[0]];
    self.renderDialog(newDialog, true);
    router.navigate('/dialog/' + newDialog._id);
};
...
Dialog.prototype.updateMedicalAppointmentRoomId = function(medicalAppointmentId, roomId){
    var self = this;
    var url = apiURLBaseJS + "dronline-patient-api/api/medical-appointment/updateRoomId/"+medicalAppointmentId+"/"+roomId;
    ...
    if (this.readyState == 4 && this.status == 200) {
        infoLog("RoomId actualizado exitosamente en BD!");
        }
    };
};
