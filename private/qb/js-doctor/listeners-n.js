...
Listeners.prototype.onMessageListener = function (userId, message) {
    ...
    var self = this,
        ...
        dialogId = dialogsList[message.dialog_id._id];
    ...
    if (dialog) {
        if(msg.attachments.length){
            logInfo(medicalIdList[message.dialog_id],"DRONLINE DOCTOR: Recibió imagen del paciente.");
        }
        else{
            logInfo(medicalIdList[message.dialog_id],"DRONLINE DOCTOR: Recibió mensaje del paciente.");
        }
        ...
        if(tabType === dialogType){
            ...
            if (dialogModule.dialogId === msg.chat_dialog_id) {
                var chatTone = new Audio('audio/1.mp3');
                chatTone.play();
                messageModule.renderMessage(msg, true);
            } else {
                var chatTone = new Audio('audio/3.mp3');
                chatTone.play();
...
Listeners.prototype.onSentMessageCallback = function (messageLost, messageSent) {
    ...
    if (messageLost) {
        ...
        data.status = parent.translationPrefered.MESSAGE_NOT_DELIVERED;
    } else {
        // message was sent to the chat but not delivered to che opponent.
        data.status = parent.translationPrefered.GENERAL_SENT;
    }
...
Listeners.prototype.onReadStatusListener = function (messageId, dialogId, userId) {
    var data = {
        ...
        dialogId: dialogId,
        userId: userId,
        status: parent.translationPrefered.MESSAGE_SEEN
    };
...
Listeners.prototype.onDeliveredStatusListener = function (messageId, dialogId, userId) {
    var data = {
        ...
        dialogId: dialogId,
        userId: userId,
        status: parent.translationPrefered.MESSAGE_DELIVERED
    };
