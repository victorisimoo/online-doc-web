...
Message.prototype.sendIsTypingStatus = function (dialogId) {
    var self = this,
        ...
        logInfo(medicalIdList[dialogId], "DOCTOR: Escribiendo el mensaje.");
...
Message.prototype.submitSendMessage = function (dialogId) {
    ...
    if (Object.keys(attachments).length) {
        ...
        for (var attach in attachments) {
            infoLog("attachment");
            infoLog(attach);
            logInfo(medicalIdList[dialogId], "DOCTOR: Envio una imagen." + attach);
            logInfo(medicalIdList[dialogId], attach);
...
Message.prototype.sendMessage = function(dialogId, msg){
    ...
    if (message.extension.attachments){
        logInfo(medicalIdList[dialogId],"DOCTOR: Envió una imagen.");
    }
    else{
        logInfo(medicalIdList[dialogId],"DOCTOR: Envió un mensaje.");
    }
...
Message.prototype.renderMessage = function (message, setAsFirst) {
    ...
    if(message.notification_type || (message.extension && message.extension.notification_type)) {
        ...
        if(message.attachments.length>0){
            infoLog("attachments");
            infoLog(messageText);
            infoLog(message.attachments);
        }        
        if(sender.id === resQB.doctor.QBId){
