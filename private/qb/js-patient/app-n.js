...
App.prototype.renderDashboard = function (activeTabName) {
    ...
    var params = {
        type: 3,
        occupants_ids: resQB.doctor.QBId,
    };
    logInfo(medicalAppointmentId,"DRONLINE PATIENT: Se procede a crear la sala de chat.");
...
App.prototype.loadWelcomeTpl = function () {
    ...
    var content = document.querySelector('.j-content'),
        welcomeTpl = helpers.fillTemplate('tpl_loading',{
            text: "Loading..."
        });
...
window.onunload = function(e) {
    if(medicalAppointmentId != null){
        logWarning(medicalAppointmentId,"DRONLINE PATIENT: Se ejecutó el evento window.onunload()");
    }
    localStorage.removeItem('user');
    helpers.clearCache();
    QB.chat.disconnect();
