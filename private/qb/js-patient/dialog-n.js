...
Dialog.prototype.renderMessages = function (dialogId) {
    ...
    if (!document.forms.send_message) {
        ...
        doctorName = resQB.doctor.name;
        self.content.innerHTML = helpers.fillTemplate('tpl_conversationContainer', {title: doctorName, _id: dialog._id, type: dialog.type});
...
Dialog.prototype.createDialog = function (params) {
    ...
    QB.chat.dialog.create(params, function (err, createdDialog) {
        if (err) {
            ...
            logInfo(medicalAppointmentId, "DRONLINE PATIENT: Chat creado exitosamente.");
            logInfo(medicalAppointmentId, "DRONLINE PATIENT: DialogId: <b>"+createdDialog._id+"</b> - Doctor QBId: <b>"+params.occupants_ids+"</b>");
            var occupants_names = [],
...
bindEvent(window, 'message', function (e) {
    if(e.data.reason == 3){
        if(e.data.doctorName != undefined && e.data.doctorName != null){
            doctorName = e.data.doctorName;
        }
        if(e.data.photo != undefined && e.data.photo != null){
