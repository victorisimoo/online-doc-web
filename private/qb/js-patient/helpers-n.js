...
Helpers.prototype.clearView = function (view) {
    var nodeList = view.childNodes; //Obtiene todos los elementos dentro del view
    for (var i = nodeList.length; i > 0; i--) {
        view.removeChild(nodeList[i - 1]); //Elimina elemento del view
    }
};
...
Helpers.prototype.getMessageStatus = function(message){
    ...
    var self = this,
        ...
        status = {
        status: !deleveredToOcuupants ? 'done_all' : readedByOcuupants ? 'done_all' : 'done',
        seen: readedByOcuupants ? true : false
        };
    return status;
};
...
Helpers.prototype.getSrcFromAttachmentId = function (id) {
    return QB.content.publicUrl(id) + '/' + '/download.xml?token=' + app.token;
};
...
Helpers.prototype.fillNewMessageParams = function (userId, msg) {
    ...
    message.status = (userId !== app.user.id) ? self.getMessageStatus(message) : { status : '', seen: false };
