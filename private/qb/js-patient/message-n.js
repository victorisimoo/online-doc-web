...
Message.prototype.sendIsTypingStatus = function (dialogId) {
    ...
    logInfo(medicalAppointmentId,"PACIENTE: Escribiendo el mensaje.");
...
Message.prototype.submitSendMessage = function (dialogId) {
    ...
    if (Object.keys(attachments).length) {
        ...
        for (var attach in attachments) {
            logInfo(medicalAppointmentId,"PACIENTE: Envio una imagen.");
...
Message.prototype.sendMessage = function(dialogId, msg){
    ...
    if (message.extension.attachments){
        logInfo(medicalAppointmentId,"PACIENTE: Envió una imagen.");
    }
    else{
        logInfo(medicalAppointmentId,"PACIENTE: Envió un mensaje.");
    }
...
Message.prototype.renderMessage = function (message, setAsFirst) {
    ...
    if(message.notification_type || (message.extension && message.extension.notification_type)) {
        ...
        messageText = (messageText == '[attachment]') ? '[imagen]' : messageText;
        if(sender.id === resQB.patient.QBId){
            messagesHtml = helpers.fillTemplate('tpl_message_me', {
                message: {
...
Message.prototype.renderTypingUsers = function (dialogId) {
    ...
    if (users.length) {
        var tpl = helpers.fillTemplate('tpl_message__typing', {users: users, photo: doctorPhoto}),
