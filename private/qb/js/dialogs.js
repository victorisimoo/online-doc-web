        ...
        if (err) {
          console.error(err);
            ...
            resDialogs.items.forEach(function(item, i, arr) {
                ...
                console.info("Joined dialog name:"+item.name);
            ...
            updateDialogsUsersStorage(jQuery.unique(occupantsIds), function(){
                // show dialogs
                resDialogs.items.forEach(function(item, i, arr) {
                    var opponentId    = QB.chat.helpers.getRecipientId(item.occupants_ids, currentUser.id);
                    if(doctorUser.id===opponentId){
                      console.log("Si debe de invocar al metodo showOrUpdateDialogInUI");                      
                      showOrUpdateDialogInUI(item, false); 
                      // and trigger the 1st dialog
                      triggerDialog(resDialogs.items[i]._id);
                    }else{
                      console.log("No debe de pintar al siguiente oponente:"+doctorUser.id+","+opponentId);
                    }
...
function showOrUpdateDialogInUI(itemRes, updateHtml) {
  ...
  if (dialogType == 3) {
    ...
    fullNameDialog = getUserNameById(opponentId);  
    dialogName    = 'Doctor '+ fullNameDialog;
    
  console.log("showOrUpdateDialogInUI:"+updateHtml);
    if (updateHtml === true) {
     
      var updatedDialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage,0);
      //$('#dialogs-list').prepend(updatedDialogHtml);
      $('#ms-body0').prepend(updatedDialogHtml);
      $('.list-group-item.active .badge').text(0).hide(0);
    } else {

      var dialogHtml = buildDialogHtml(dialogId, dialogUnreadMessagesCount, dialogIcon, dialogName, dialogLastMessage,1);
      //$('#dialogs-list').append(dialogHtml);
      //$('#ms-body0').append(dialogHtml);
      
      //$(".ms-user").html('<img src="../img/profile-pics/1.jpg" alt=""><div>Paciente: <br/>' + currentUser.name + '</div>');
      //$(".ms-user-header").html('<div class="lv-avatar pull-left"><img src="../img/profile-pics/4.jpg" alt=""></div>' + '<span class="c-black">Doctor: <strong>' + fullNameDialog + '</strong></span>');
...
// create new dialog
function createNewDialog(user1,user2) {
  var usersIds = [];
  var usersNames = [];
  ...
  usersIds[0] = user2.id;
  //usersIds[1] = user2.id;
  usersNames[0] = user2.name;
  //usersNames[1] = user2.name;
  ...
  QB.chat.dialog.create(params, function(err, createdDialog) {
    if (err) {
      ...
      //showOrUpdateDialogInUI(createdDialog, false);
