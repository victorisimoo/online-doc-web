...
function setupMsgScrollHandler() {
  ...
  console.log("setupMsgScrollHandler:true");
  ...
  msgList.scroll(function() {
    if (msgWindow.scrollTop() == msgWindow.height() - msgList.height()){
      ...
      console.log("setupMsgScrollHandler:false");
...
function onMessage(userId, msg) {
  ...
  if (isMessageForCurrentDialog(userId, msg.dialog_id)){
    ...
    console.log("userId:"+userId+";currentUser.id:"+currentUser.id);
    ...
    QB.chat.message.list(params, function(err, messages) {
      if (messages) {
        ...
        messages.items.forEach(function(item, i, arr) {
          ...
          var messageSenderLogin = getUserNameById(messageSenderId);          
          if(currentUser.id===messageSenderId){
              itmyself = 1;
          }
          ...
          var messageHtml = buildMessageHTML(messageText, messageSenderLogin, messageDateSent, messageAttachmentFileId, messageId,1,itmyself);

           //$('#messages-list').prepend(messageHtml);
           $('#ms-body0').prepend(messageHtml);

          //var html,avatar;

           //html = '<div class="lv-item media right">';
           //avatar = '<div class="lv-avatar pull-right">';
           /*selector = $('#ms-body' + 0);
           if(itmyself===1){
              html = '<div class="lv-item media right">';
              avatar = '<div class="lv-avatar pull-right">';
              avatar += '<img src="../img/profile-pics/4.jpg" alt="">';
           }else{
              html = '<div class="lv-item media">';
              avatar = '<div class="lv-avatar pull-left">';
              avatar += '<img src="../img/profile-pics/1.jpg" alt="">';
           }
           html += avatar;
           html += '</div> <div class="media-body"><div class="ms-item">' + messageText + '</div>';
           html += '</div> </div>';
            selector.prepend(html);
            var objDiv = document.getElementById("ms-body" + 0);
...
function showMessage(userId, msg, attachmentFileId) {
  ...
  var messageHtml = buildMessageHTML(msg.body, userLogin, new Date(), attachmentFileId, msg.id,1,itmyself);

   /*var html = '<div class="lv-item media right">';
           selector = $('#ms-body' + 0);
           html += '</div> <div class="media-body"><div class="ms-item">' + msg.body + '</div>';
           html += '</div> </div>';
            selector.append(html);*/

  //$('#messages-list').append(messageHtml);
  console.log("itmyself in showMessage is:"+itmyself)
  $('#ms-body0').append(messageHtml);
