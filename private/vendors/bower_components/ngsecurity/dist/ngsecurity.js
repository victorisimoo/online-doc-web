...
//render class
    ...
    function securityFactory($rootScope, $cookies, $q, $http, $securityConfig) {
        ...
        function authStrategySimple(token, user, permissions) {
            var tokenBearer = $securityConfig.token.prefix + token;
            var config = {
                secure: true
            };
            $cookies.put($securityConfig.storageName.token, tokenBearer, config);
            $cookies.putObject($securityConfig.storageName.user, user, config);
            $cookies.putObject($securityConfig.storageName.permissions, permissions, config);
        }
    ...
    function securityInterceptor($rootScope, $q, $cookies, $securityConfig) {
        ...
        function response(response) {
            if (typeof response.headers('refresh_token') !== 'undefined' && response.headers('refresh_token') !== null) {
                //$cookies.remove($securityConfig.storageName.token);
                //$cookies.put($securityConfig.storageName.token, 'Bearer ' + response.headers('refresh_token'));
            }
